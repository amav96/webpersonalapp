...
export function UserItem(props) {
    ...
    const openDesactivateActivateConfirm = () => {
        setIsDelete(false);
        setConfirmMessage(`${user.active ? 'Desactivar' : 'Activar'} usuario ${user.email}`)
        onOpenCloseConfirm();
    }

    const onActivateDesactivate = async () => {
        try {
            await userController.update(accessToken, user._id, {
                active: !user.active
            });
            onReload();
            onOpenCloseConfirm();
    ...
    const onDelete = async () => {
        console.log("gege")
        try {
            await userController.remove(accessToken, user._id);
            onReload();
            onOpenCloseConfirm();
            setIsDelete(false);
        } catch (error) {
